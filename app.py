...
def corr_matrix(df, esg_cols, scm_cols):
    ...
    for e in esg_cols:
        ...
        for s in scm_cols:
            sub = df[[e, s]].dropna()
            val = sub[e].corr(sub[s]) if len(sub) > 2 else np.nan
            row.append(round(val, 3) if pd.notna(val) else np.nan)
        rows.append(row)
...
# ─────────────────────────────────────────────────────────────
    ...
    if not esg or not scm:
        st.error("Required ESG/SCM columns not found")
    else:
...
# ─────────────────────────────────────────────────────────────
    ...
    if df_raw is None or "COMPANY" not in df_raw.columns:
        st.warning("Excel must contain COMPANY column")
        st.stop()
...
# ─────────────────────────────────────────────────────────────
    ...
    for c in companies:
        ...
        cls = f"alert-card-{risk.lower()}" if risk else "alert-card-low"
...
# ─────────────────────────────────────────────────────────────
    ...
    if df_raw is None or "YEAR" not in df_raw.columns:
        st.warning("Excel must contain YEAR column")
        st.stop()
